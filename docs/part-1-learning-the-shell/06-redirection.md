...
与管道的隐喻相匹配，Linux 提供一个 `tee` 的命令创造了一个「三通」来适配我们的「管道」。`tee` 程序读取标准输入，复制到标准输出（并允许数据继续在管道中流向）和一个或多个文件。对于捕捉即时处理中的管道中的内容会很有用。来重复一个之前的示例，这次在 `grep` 之前，用 `tee` 来捕捉整个目录，并将其导向 `ls.txt` 文件：
...
和之前一样，检查一下本章中我们提及的每个命令。我们只是看到了它们最基本的用法而已。它们都有一大堆有趣的选项。当我们获得 Linux 的体验时，我们会看到命令行的重定向特征是某些特殊问题的极其有效的解决方法。很多命令会使用标准输入输出，绝大部分的命令行程序会使用标准错误来显示它们的信息。

> **Linux 关乎想象**
> 
> 当我被问及如何解释 Windows 和 Linux 之间的差异时，我经常使用一个关于玩具的比喻。
> 
> Windows 像是一个游戏男孩。你到商店里买了一个崭新玩具，拿回家，打开，玩耍。它有漂亮的图形界面，好听的声音。然而过了不久，你厌倦了这个游戏所带给你的感觉，于是你回到商店，又买了另外一个玩具。这个循环一遍又一遍，最终，你回到商店，对店员说「我想要一个能这么玩的游戏！」。而你会被告知，因为没有「市场需求」，你要的游戏不存在。然后你说「但是我仅仅想换一件东西！」店员会告诉你，不能换。游戏都是被封装在它们的盒子里的。你发现你的玩具受限于其他人决定你需要什么样的游戏。
> 
> 另一方面，Linux 像是世界上最大的竖立装置。你打开它，它仅仅是一大堆零件。有一大堆的支撑架、螺母、螺丝、齿轮、滑轮、马达，还有一些如何搭建的参考意见。于是你开始玩。按这个意见造了一个，然后又按照另一个意见再造一个。过了一会儿，你发现你有了自己的想法。你不必回到商店，因为你已经有了所需的一切。这个竖立装置依赖于你的想象，按你所需而工作。
> 
> 当然，你如何挑选玩具完全是你的个人事务，所以，哪个玩具你会更满意呢？
